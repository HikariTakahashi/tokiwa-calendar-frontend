export const useTwitterAuth = () => {
  ...
  const getTwitterAuthUrl = (redirectUri: string, state?: string): string => {
    ...
    // stateパラメータがある場合は追加、ない場合はデフォルト値を使用
    if (state) {
  ...
  const authenticateWithTwitter = async (
    ...
    try {
      const response = await twitterAuth(code, redirectUri, linkUID);
  ...
  // Twitter認証フローを開始
  const startTwitterAuth = (redirectUri: string, state?: string): void => {
    try {
